...
考虑情况：文件a中定义了字符串s，文件b通过extern关键字使用该字符串，a和b被分别编译并链接成可执行文件。此时如果进行字符串加密，则s在a中被加密，但b中无法添加相应的解密代码，程序运行会出错（出现乱码）。

C/C++语言中的 linkage 表示了符号是否对编译单元（translation unit）外可见。编译器默认的 linkage 类型为：

- Non-const global variables：默认为 external linkage

- Const global variables：默认为 internal linkage

- 函数：默认为 external linkage

因此，这种情况下是无法从文件a判断是否要对s进行加密的。解决方案：1）不对全局字符串加密；2）对程序整体在LTO阶段使用字符串加密。

> 参考：
>
> http://www.goldsborough.me/c/c++/linker/2016/03/30/19-34-25-internal_and_external_linkage_in_c++/
>
> *LLVM Documentation-Linkage Types*
>
